...
import { isWithinWindow } from "@/hooks/useAppointmentChat";
...
const ClientChat = () => {
  ...
  const { data: conversations = [], isLoading } = useQuery({
    queryKey: ["client_chat_list", user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      const { data: apts, error } = await supabase
        .from("appointments")
        .select("id, scheduled_at, duration_minutes, status, lawyer_id")
      ...
      const lawyerIds = Array.from(new Set(apts.map(a => a.lawyer_id)));
      ...
      return apts.map((a: any) => {
        const profile = profiles.find(p => p.user_id === a.lawyer_id);
        const now = new Date();
    ...
    enabled: !!user?.id,
    refetchInterval: 30000, // re-check live status every 30s
  });
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 border-r flex flex-col shrink-0">
          <div className="p-4 border-b">
            ...
            <p className="text-xs text-muted-foreground mt-1">Chat is live only during your appointment window</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="p-6 text-center text-muted-foreground text-sm">
                ...
                No appointments yet. Book a lawyer to start chatting.
              </div>
            ) : (
              conversations.map((c) => (
                <button
                ...
                >
                  <div className="relative">
                    <Avatar className="h-10 w-10">
                      <AvatarFallback>{c.lawyer_name.split(" ").map(n => n[0]).join("").slice(0, 2)}</AvatarFallback>
                    </Avatar>
                    {c.isLive && (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">{c.lawyer_name}</p>
          ...
          {selected ? (
            <>
              ...
              <div className="p-4 border-b flex items-center gap-3 bg-card">
                <Avatar className="h-9 w-9">
                  <AvatarFallback>{selected.lawyer_name.split(" ").map(n => n[0]).join("").slice(0, 2)}</AvatarFallback>
                </Avatar>
                <div>
                  <p className="font-semibold">{selected.lawyer_name}</p>
              ...
              <div className="flex-1 min-h-0">
                <ChatPane
                  ...
                  otherUserName={selected.lawyer_name}
                />
              </div>
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-muted-foreground">
              ...
              <p className="text-sm max-w-sm">
                Choose a lawyer from the list. The chat will unlock automatically when your appointment window starts.
              </p>
            </div>
          )}
        </div>
